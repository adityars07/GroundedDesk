...
1. Make sure your root `.env` file has a valid `OPENAI_API_KEY`.
...
2. Pre-embed the local knowledge base chunks.
3. Simulate semantic retrieval using cosine similarity.
4. Call `gpt-4o` with the retrieved context chunks to generate RAG completions.
5. Grade the quality metrics using `gpt-4o-mini` as an evaluator judge.
